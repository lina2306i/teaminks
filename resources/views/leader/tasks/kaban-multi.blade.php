...
@section('contentW')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        @endif
        <div class="btn-group">
            <button class="btn btn-outline-light active" data-view="kanban">Kanban</button>
            <button class="btn btn-outline-light" data-view="list">List</button>
            <button class="btn btn-outline-light" data-view="calendar">Calendar</button>
        </div>
    </div>
    <!-- Vue Kanban (ton code existant avec drag & drop) -->
    <div id="kanban-view" class="view-active row g-4">
        ...
        <table class="table table-dark table-hover">
            ...
            <tbody>
                @foreach($tasks as $task)
                    <tr>
                        <td>{{ $task->title }}</td>
                        <td><span class="badge bg-{{ $task->status == 'completed' ? 'success' : 'warning' }}">{{ ucfirst($task->status) }}</span></td>
    ...
    <div id="calendar-view" class="d-none">
        <div id="calendar" class="border border-gray-700 rounded p-3 bg-gray-800"></div>
    </div>
...
document.querySelectorAll('[data-view]').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('#kanban-view, #list-view, #calendar-view').forEach(view => view.classList.add('d-none'));
        document.getElementById(btn.dataset.view + '-view').classList.remove('d-none');
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: @json($tasks->map(function($t) {
    return [
        'title' => $t->title,
        'start' => $t->start_date ? $t->start_date->toIso8601String() : null,
        'end'   => $t->end_date ? $t->end_date->toIso8601String() : null,
        'color' => $t->status == 'completed' ? '#10b981' : '#ef4444',
    ];
})),
        eventClick: info => {
            alert('Tâche : ' + info.event.title);
            // ou : window.location = '/leader/tasks/' + info.event.extendedProps.taskId;
        }
    });
    calendar.render();
});
